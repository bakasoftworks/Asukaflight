...
void RadioTrainerPage::build(Window* form)
{
  ...
  if (SLAVE_MODE()) {
    ...
    auto statsLine = form->newLine(grid);
    new StaticText(statsLine, rect_t{}, "SBUS RX/s");
    new DynamicNumber<uint16_t>(
        statsLine, rect_t{}, []() { return trainerGetRxRateHz(); },
        COLOR_THEME_PRIMARY1_INDEX, LEFT);
    new StaticText(statsLine, rect_t{}, "Mix samp/s");
    new DynamicNumber<uint16_t>(
        statsLine, rect_t{}, []() { return trainerGetSampleRateHz(); },
        COLOR_THEME_PRIMARY1_INDEX, LEFT);
