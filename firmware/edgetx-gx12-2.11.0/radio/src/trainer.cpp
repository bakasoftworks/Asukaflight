...
static uint16_t trainerInputActiveMask = 0xffff;
static uint16_t trainerInputHighResolutionMask = 0x0000;

void (*_on_change_cb)(uint8_t, uint8_t) = nullptr;

static volatile uint32_t trainerRxFrames = 0;
static volatile uint32_t trainerSampledFrames = 0;
static volatile uint32_t trainerFrameSeq = 0;
static uint32_t trainerLastSampledFrameSeq = 0;
static uint16_t trainerRxFramesWindow = 0;
static uint16_t trainerSampledFramesWindow = 0;
static uint16_t trainerRxRateHz = 0;
static uint16_t trainerSampleRateHz = 0;
static uint8_t trainerStatsTicks = 0;

// Active signal received
bool isTrainerValid() { return trainerInputValidityTimer != 0; }

bool isTrainerChannelActive(uint8_t channel)
{
  return channel < MAX_TRAINER_CHANNELS &&
         (trainerInputActiveMask & (1U << channel)) != 0;
}

bool isTrainerChannelHighResolution(uint8_t channel)
{
  return channel < MAX_TRAINER_CHANNELS &&
         (trainerInputHighResolutionMask & (1U << channel)) != 0;
}

void trainerSetActiveMask(uint16_t mask)
{
  trainerInputActiveMask = mask;
}

void trainerSetHighResolutionMask(uint16_t mask)
{
  trainerInputHighResolutionMask = mask;
}
...
void trainerDecTimer()
{
  ...
  if (++trainerStatsTicks >= 100) {
    trainerRxRateHz = trainerRxFramesWindow;
    trainerSampleRateHz = trainerSampledFramesWindow;
    trainerRxFramesWindow = 0;
    trainerSampledFramesWindow = 0;
    trainerStatsTicks = 0;
  }
}
...
void trainerNoteSbusFrame()
{
  trainerRxFrames++;
  trainerRxFramesWindow++;
  trainerFrameSeq++;
}

void trainerNoteSourceRead()
{
  uint32_t frameSeq = trainerFrameSeq;
  if (frameSeq != 0 && frameSeq != trainerLastSampledFrameSeq) {
    trainerLastSampledFrameSeq = frameSeq;
    trainerSampledFrames++;
    trainerSampledFramesWindow++;
  }
}

uint16_t trainerGetRxRateHz() { return trainerRxRateHz; }

uint16_t trainerGetSampleRateHz() { return trainerSampleRateHz; }

uint32_t trainerGetRxFrames() { return trainerRxFrames; }

uint32_t trainerGetSampledFrames() { return trainerSampledFrames; }
