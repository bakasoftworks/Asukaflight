...
extern struct usbReport_t usbReportDesc(void);
extern uint8_t usbReportSize(void);
#endif
...
#if defined(USBJ_EX)
      pdev->tclasslist[pdev->classId].CurrPcktSze = usbReportSize();
#else
      pdev->tclasslist[pdev->classId].CurrPcktSze = HID_IN_PACKET;
#endif
...
#if defined(USBJ_EX)
  pHidMouseDesc->wItemLength = usbReportDesc().size;
#else
  pHidMouseDesc->wItemLength = HID_JOYSTICK_REPORT_DESC_SIZE;
#endif
  *Sze += (uint32_t)sizeof(USBD_HIDDescTypeDef);

  /* Append Endpoint descriptor to Configuration descriptor */
#if defined(USBJ_EX)
  __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add, USBD_EP_TYPE_INTR, usbReportSize(), \
                       1U, 1U);
#else
  __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add, USBD_EP_TYPE_INTR, HID_IN_PACKET, \
                       1U, 1U);
#endif
