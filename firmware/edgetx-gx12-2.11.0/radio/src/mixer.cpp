...
int16_t trainerInputToResx(uint8_t channel, int16_t calib = 0)
{
  int32_t value = trainerInput[channel];
  if (isTrainerChannelHighResolution(channel)) {
    value -= (int32_t)calib * 2;
  }
  else {
    value -= calib;
    value *= 2;
  }
  return limit<int16_t>(-RESX, value, RESX);
}
...
void applyExpos(int16_t * anas, uint8_t mode, int16_t ovwrIdx, int16_t ovwrValue)
{
  ...
  for (uint8_t i=0; i<MAX_EXPOS; i++) {
    ...
    if (ed->flightModes & (1<<mixerCurrentFlightMode))
      continue;
    if (src >= MIXSRC_FIRST_TRAINER && src <= MIXSRC_LAST_TRAINER &&
        (!isTrainerValid() || !isTrainerChannelActive(src - MIXSRC_FIRST_TRAINER)))
      continue;
...
int16_t applyLimits(uint8_t channel, int32_t value)
{
  ...
  if (isFunctionActive(FUNCTION_TRAINER_CHANNELS) && isTrainerValid() &&
      isTrainerChannelActive(channel)) {
    return trainerInputToResx(channel);
  }
...
getvalue_t _getValue(mixsrc_t i, bool* valid)
{
  if (i == MIXSRC_NONE) {
    ...
    uint8_t trainerChannel = i - MIXSRC_FIRST_TRAINER;
    if (!isTrainerChannelActive(trainerChannel)) {
      if (valid != nullptr) *valid = false;
      return 0;
    }
    trainerNoteSourceRead();
    int16_t calib = 0;
    if (i < MIXSRC_FIRST_TRAINER + NUM_CAL_PPM) {
      calib = g_eeGeneral.trainer.calib[trainerChannel];
    }
    return trainerInputToResx(trainerChannel, calib);
    ...
    if (ch < pots_offset) { // only do this for sticks
      ...
      if (mode <= e_perout_mode_inactive_flight_mode &&
        ...
        if (td->mode) {
          ...
          if (isTrainerChannelActive(chStud)) {
            int32_t vStud = trainerInputToResx(chStud,
                                               g_eeGeneral.trainer.calib[chStud]);
            vStud *= td->studWeight;
            vStud /= 100;
            switch (td->mode) {
              case TRAINER_ADD:
