...
static int8_t VCP_Init_FS(void)
{
  cdcConnected = true;
#if defined(USE_USBD_COMPOSITE)
  USBD_CDC_SetTxBuffer(&hUsbDevice, UserTxBufferFS, 0, USB_CDC_COMPOSITE_CLASS_ID);
#else
...
#if defined(USE_USBD_COMPOSITE)
      (USBD_CDC_HandleTypeDef*)hUsbDevice.pClassDataCmsit[USB_CDC_COMPOSITE_CLASS_ID];
#else
...
#if defined(USE_USBD_COMPOSITE)
    USBD_CDC_SetTxBuffer(&hUsbDevice, &UserTxBufferFS[APP_Tx_ptr_out], length, USB_CDC_COMPOSITE_CLASS_ID);
    result = USBD_CDC_TransmitPacket(&hUsbDevice, USB_CDC_COMPOSITE_CLASS_ID);
#else
