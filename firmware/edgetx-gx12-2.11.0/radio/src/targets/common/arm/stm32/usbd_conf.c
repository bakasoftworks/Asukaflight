...
USBD_StatusTypeDef USBD_LL_Init(USBD_HandleTypeDef *pdev)
{
  ...
  if (pdev->id == DEVICE_FS) {
    ...
    HAL_PCDEx_SetRxFiFo(&hpcd_USB_OTG, RX_FIFO_FS_SIZE);
    HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG, 0, TX0_FIFO_FS_SIZE);
    HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG, 1, TX1_FIFO_FS_SIZE);
    HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG, 2, TX2_FIFO_FS_SIZE);
    HAL_PCDEx_SetTxFiFo(&hpcd_USB_OTG, 3, TX3_FIFO_FS_SIZE);
  }
#endif
  return USBD_OK;
}
