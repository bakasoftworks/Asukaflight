...
#if defined(USE_USBD_COMPOSITE) && !defined(BOOT)
static constexpr uint8_t USB_CDC_COMPOSITE_CLASS_ID = 0;
static constexpr uint8_t USB_HID_COMPOSITE_CLASS_ID = 1;
static constexpr uint8_t USB_MSC_COMPOSITE_CLASS_ID = 0;
static uint8_t cdcEpAdd[] = {0x81, 0x01, 0x82};
static uint8_t hidEpAdd[] = {0x83};
static uint8_t mscEpAdd[] = {0x81, 0x01};
#endif
...
void usbStart()
{
  ...
  switch (getSelectedUsbMode()) {
    case USB_MASS_STORAGE_MODE:
      ...
      usbInitLUNs();
#if defined(USE_USBD_COMPOSITE) && !defined(BOOT)
      USBD_RegisterClassComposite(&hUsbDevice, &USBD_MSC, CLASS_TYPE_MSC, mscEpAdd);
      hUsbDevice.classId = USB_MSC_COMPOSITE_CLASS_ID;
#else
    ...
    case USB_JOYSTICK_MODE:
      // initialize USB as HID joystick, with CDC enabled on the same cable
#if defined(USBJ_EX)
      setupUSBJoystick();
#endif
#if defined(USE_USBD_COMPOSITE)
      USBD_RegisterClassComposite(&hUsbDevice, &USBD_CDC, CLASS_TYPE_CDC, cdcEpAdd);
      USBD_RegisterClassComposite(&hUsbDevice, &USBD_HID, CLASS_TYPE_HID, hidEpAdd);
      hUsbDevice.classId = USB_CDC_COMPOSITE_CLASS_ID;
...
#if defined(USE_USBD_COMPOSITE)
   USBD_HID_SendReport(&hUsbDevice, HID_Buffer, HID_IN_PACKET, USB_HID_COMPOSITE_CLASS_ID);
#else
...
#if defined(USE_USBD_COMPOSITE)
  USBD_HID_SendReport(&hUsbDevice, ret.ptr, ret.size, USB_HID_COMPOSITE_CLASS_ID);
#else
