...
public static class OptionTooltipCatalog
{
    ...
    private static readonly IReadOnlyDictionary<string, string> BindingSettings =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["ElasticReturnMetric"] = "mapper.elastic_return_metric",
            ["ElasticReturnActivation"] = "mapper.elastic_return_activation",
            ["ElasticReturnCoefficient"] = "mapper.elastic_return_coefficient",
            ["ElasticReturnIdleCoefficient"] = "mapper.elastic_return_idle_coefficient",
            ["ElasticReturnTaperMs"] = "mapper.elastic_return_taper_ms",
            ...
            ["RightMouseLeftThrottleStep"] = "right_mouse_left_stick.throttle_step",
            ["RightMouseLeftThrottleButtonRate"] = "right_mouse_left_stick.throttle_button_rate",
            ["RightMouseLeftThrottleReturnEnabled"] = "right_mouse_left_stick.throttle_return_enabled",
            ["RightMouseLeftThrottleReturnRate"] = "right_mouse_left_stick.throttle_return_rate",
            ["RightMouseLeftYawPulse"] = "right_mouse_left_stick.yaw_pulse",
            ["RightMouseLeftYawScrollStep"] = "right_mouse_left_stick.yaw_scroll_step",
            ["RightMouseLeftYawSlewRate"] = "right_mouse_left_stick.yaw_slew_rate",
            ["RightMouseLeftInvertThrottle"] = "right_mouse_left_stick.invert_throttle",
            ["RightMouseLeftInvertYaw"] = "right_mouse_left_stick.invert_yaw",
            ["RightMouseLeftSwapAxes"] = "right_mouse_left_stick.swap_axes",
    ...
    private static readonly IReadOnlyDictionary<string, OptionTooltipInfo> BindingTooltips =
        new Dictionary<string, OptionTooltipInfo>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["LeftStickSource"] = new(
                ...
                "Off keeps throttle low and yaw centered. Keyboard / Wooting enables the keyboard source. Second mouse enables the bound left mouse source. Right Mouse Mouse4/Mouse5 + Scroll uses the right mouse side buttons and wheel. The launcher saves the underlying source booleans atomically so left-stick sources do not overlap.",
    ...
    private static readonly IReadOnlyDictionary<string, string> SettingHelpOverrides =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            ["safety.stop_key"] = "Global key that starts the selected profile when idle and stops the active managed run.",
            ...
            ["mapper.elastic_return_metric"] = "Axis keeps independent roll/pitch return. Diagonal normalized applies one 2D return vector to avoid stronger diagonal pullback.",
            ["mapper.elastic_return_activation"] = "Always keeps elastic return active continuously. While moving uses Elastic coeff while mouse input is present, then fades toward Idle coeff after input stops.",
            ["mapper.elastic_return_idle_coefficient"] = "Target elastic coefficient after mouse input stops while Spring timing is While moving. Zero preserves the old full cutoff.",
            ["mapper.elastic_return_taper_ms"] = "How long While moving takes to fade from the movement coefficient to the idle coefficient after mouse input stops.",
            ...
            ["right_mouse_left_stick.enabled"] = "Uses the right mouse side buttons and vertical wheel as the left-stick source.",
            ["right_mouse_left_stick.throttle_step"] = "Trainer throttle units added per wheel notch when scroll controls throttle.",
            ["right_mouse_left_stick.throttle_button_rate"] = "Trainer throttle units per second when Mouse4/Mouse5 control throttle in swapped mapping.",
            ["right_mouse_left_stick.throttle_return_enabled"] = "Returns throttle toward the low-throttle safety value when the active throttle input is idle.",
            ["right_mouse_left_stick.throttle_return_rate"] = "Throttle return speed in trainer units per second.",
            ["right_mouse_left_stick.yaw_pulse"] = "Maximum left-yaw output for Mouse4/Mouse5 or accumulated scroll yaw.",
            ["right_mouse_left_stick.yaw_scroll_step"] = "Trainer yaw units added per wheel notch when scroll controls yaw in swapped mapping.",
            ["right_mouse_left_stick.yaw_slew_rate"] = "Yaw output slew limit for the right-mouse button/scroll source.",
            ["right_mouse_left_stick.invert_throttle"] = "Reverses the throttle direction for the right-mouse button/scroll source.",
            ["right_mouse_left_stick.invert_yaw"] = "Reverses the yaw direction for the right-mouse button/scroll source.",
            ["right_mouse_left_stick.swap_axes"] = "Swaps the mapping so Mouse4/Mouse5 control throttle and scroll controls yaw.",
